...
function getHoursDifference(deadline) {
  ...
  return hoursDiff < 0
    ? "Quá hạn"
    : hoursDiff < 24
    ? `${Math.floor(hoursDiff)} giờ `
    : `${Math.floor(hoursDiff / 24)} ngày`;
}
...
export default function ManagementTaskByOrder() {
  ...
  const ManagementTasksContent = () => {
    ...
    const fetchTemplates = async () => {
      try {
        const response = await fetch(
          "https://localhost:7259/api/task/dashboard",
          {
            method: "GET",
            headers: {
    ...
    const fetchAllStaff = async () => {
      try {
        const response = await fetch(
          "https://localhost:7259/api/staff/current-tasks",
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    useEffect(() => {
      ...
      const fetchAllMaterial = async () => {
        const GET_ALL_MATERIAL_URL = `https://localhost:7259/api/material`;
        try {
    ...
    const Content = ({ product, stage, materials }) => {
      ...
      const handleOk = async (productId) => {
        if (productId && selectedValue !== null) {
          ...
          const manager = JSON.parse(localStorage.getItem("manager"));
          try {
            const URL = `https://localhost:7259/api/task/staff/${selectedValue}/assign/${productId}`;
      ...
      const handleDeadlineUpdate = async (productId, newDeadline) => {
        ...
        const manager = JSON.parse(localStorage.getItem("manager"));
        const UPDATE_DEADLINE_URL = `https://localhost:7259/api/task/task/${productId}/deadline?deadlineTickString=${newDeadline}`;
        try {
        ...
        <div style={{ marginLeft: 10, minWidth: 240 }}>
          <div>
            <div style={{ display: "flex" }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 600, paddingLeft: 30 }}>
                Tiến độ:{" "}
                <span style={{ fontWeight: 400 }}>
                  {product.productStages[0].stageNum}/
                  {currentTemplate?.templateStages?.length}
                </span>
              </p>
            </div>
              ...
              {deadlineUpdateLoading ? (
                ...
                <div style={{ marginTop: 20, width: "100%" }}>
                  ...
                  <Row style={{ width: "100%", justifyContent: "flex-end" }}>
                    <Col span={12}>
                      <p style={{ fontSize: 15, fontWeight: 600, margin: 10 }}>
                        Ghi chú:{" "}
                        <span style={{ fontWeight: 400 }}>
                          {product?.note ? product.note : "Không có ghi chú"}
                        </span>{" "}
                    ...
                    <Col span={12}>
                      ...
                      <div>
                        <p
                          style={{ fontSize: 15, fontWeight: 600, margin: 10 }}
                        >
                          Màu vải:
                        </p>
                  ...
                  >
                    <p style={{ fontSize: 15, fontWeight: 600 }}>
                      Nhân viên thực hiện : {product?.staffMaker?.fullname}
                    </p>
                    <Button
                      style={{ marginLeft: 20, border: "none" }}
                      onClick={() => setChangeClick(changeClick ? false : true)}
                    >
                      Thay đổi
                    </Button>
                  </div>
          ...
          <>
            ...
            >
              ...
              >
                ...
                <div style={{ padding: 5 }}>
                  {templatesData &&
                    templatesData?.map((category, index) => (
                      <div
                        style={{
                      ...
                      >
                        <Accordion
                          disabled={category.totalTask === 0 ? true : false}
                        >
                          <AccordionSummary
                            sx={{
                              backgroundColor:
                                category.totalTask === 0
                                  ? "rgba(217, 217, 217, 0.432)"
                                  : "#9F78FF",
                            }}
                            expandIcon={
                              <ExpandMoreIcon
                          ...
                          >
                            <p
                              className="template-category-text"
                              style={{
                                color:
                                  category.totalTask === 0
                                    ? "#000000"
                            ...
                            >
                              {" "}
                              ( {category.totalTask} ) {category.name}
                            </p>
                          </AccordionSummary>
                          {category?.productTemplates?.map(
                            (template, index) => (
                              <>
                                <AccordionDetails
                                  expanded={expanded}
                                  sx={{ cursor: "pointer" }}
                                  onClick={() =>
                                    handleChoseTemplate(template.id)
                                  }
                                  key={template.id}
                                >
                                  <p
                                    className="template-product-text"
                                    style={{
                                      color:
              ...
              >
                <div
                  style={{
                    display: "grid",
                    height: "70vh",
                    gridTemplateColumns: " repeat(minmax(300px,300px))",
                    gridAutoFlow: "column",
                    paddingLeft: 20,
                    paddingRight: 20,
                    marginTop: 20,
                    ...
                    >
                      ...
                      >
                        {currentTemplate?.products &&
                          currentTemplate?.products.map((product, index) => {
                            if (
                              product?.productStages[0]?.stageNum ===
                              stage?.stageNum
                            ) {
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <span style={{ fontWeight: 400 }}>
                                          {product.productStages[0].stageNum}/
                                          {
                                            currentTemplate?.templateStages
                                              ?.length
                                          }
                                        </span>
                                      </p>
                                      ...
                                      <div
                                        title={product?.staffMaker?.fullname}
                                        style={{
                                          position: "absolute",
                                          bottom: 2,
                                          right: 2,
