...
export default function ManagementTask() {
  ...
  const ManagementTasksContent = () => {
    ...
    const getAllTasksUrl = "https://localhost:7259/api/task/manager/get-all";
    ...
    const handleGetAllTasks = async () => {
      ...
      try {
        ...
        if (response.ok && response.status === 200) {
          ...
          const filteredTasks = responseData.filter(
            (task) => task.staffMakerId === null
          );
          setUnAssignedTasks({
            ...unAssignedTasks,
            data: filteredTasks,
    ...
    const handleDragEnd = async (result) => {
      ...
      const DndTaskURL = `https://localhost:7259/api/task/swap-task/${result.draggableId}?staffId=${result.destination.droppableId}&index=${result.destination.index}`;
    ...
    const handleAutoAssign = async () => {
      setLoading(true);
      const URL = `https://localhost:7259/api/task/auto/create-and-assign-task`;
          ...
          <div style={{ display: "flex" }}>
            <DragDropContext onDragEnd={handleDragEnd}>
              ...
              >
                ...
                <Droppable
                  key={unAssignedTasks?.id}
                  droppableId={unAssignedTasks?.id}
                >
                  {(provided) => (
                    <div
                      ...
                      <div style={{ color: "#000000", fontWeight: "600" }}>
                        ...
                        >
                          {unAssignedTasks?.data.length === 0 ? (
                            ...
                            >
                              {unAssignedTasks?.data.length > 0 &&
                                unAssignedTasks?.data.map((task, index) => (
                                  <Popover
                                    content={content}
                                    title="Title"
                                    trigger="click"
                        ...
                        >
                          <button
                            style={{
                              ...
                              height: "1rem",
                              backgroundColor:
                                getStatusTextAndColor(index).backgroundColor,
                              borderRadius: 5,
                              border: `1px solid ${
                                getStatusTextAndColor(index).borderColor
                              }`,
                            }}
                          ></button>
                          &nbsp;
                          <p
                            style={{
                              fontSize: 15,
                              color: getStatusTextAndColor(index).color,
                            }}
                          >
                            {getStatusTextAndColor(index).text}
                          </p>
                        </div>
                      ))}
                    </div>
                                    ...
                                    <h2>
                                      ...
                                      <span
                                        style={{
                                          ...
                                          marginLeft: 10,
                                          borderRadius: 2,
                                          border: "0.8px solid #f3f5fa",
                                        }}
                                      >
                                        {allTasks &&
                                          allTasks.filter(
                                            (task) =>
                                              task?.staffMaker?.id === staff.id
                                          ).length}
                                      </span>
                                    </h2>
                                  </div>
