...
export default function BlogDetail() {
  ...
  console.log("ID", id);
  const blogDetailUrl = `https://localhost:7259/api/blog/${id}`;
  const { data: blog, isLoading: loading } = useBlogDetailQuery(blogDetailUrl);
  const queryClient = useQueryClient();

  const handleRelativeBlog = async (id) => {
    const blogRelativeDetailUrl = `https://localhost:7259/api/blog/${id}`;
  ...
  useEffect(() => {
    const fetchRelativeBlog = async () => {
      ...
      const RELATIVE_BLOG_URL = `https://localhost:7259/api/blog/relative?hastag=${encodedHashtag}`;
            ...
            <div className="blog-wrapper-container">
              ...
              <div style={{ position: "relative", minHeight: "100vh" }}>
                <div style={{ position: "absolute", right: 10 }}>
                  <div
                    style={{ position: "fixed", top: "140px", right: "10px" }}
                  >
                    <div
                      style={{
                    ...
                    {relativeBlog.length > 0 &&
                      relativeBlog.map((blog, index) => (
                        <div key={index}>
                          <div
                            className="relative-blog"
                            style={{ display: "flex", alignItems: "center" }}
                            onClick={() => handleRelativeBlog(blog.id)}
                          >
                            <img
                              src={blog?.thumbnail}
                              alt=""
