...
export default function OrderDetail() {
                ...
                >
                  ...
                  >
                    <textarea
                      ...
                      value={chat}
                      disabled={
                        orderDetails.status === 0 || orderDetails.status === 7
                          ? true
                          : false
                      }
                      maxLength={500}
                      ...
                      onKeyDown={(e) =>
                        e.key === 13 && e.shiftKey === false
                          ? handleSendChat(orderId)
                          : null
                      }
                    ></textarea>
  ...
  const fetchChat = async () => {
    const customer = localStorage.getItem("customer");
    const token = JSON.parse(customer)?.token;
    const GET_CHAT_API = `https://localhost:7259/api/chat/order/${id}`;
    try {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        ...
        const customer = localStorage.getItem("customer");
        const token = JSON.parse(customer)?.token;
        const response = await fetch(`https://localhost:7259/api/order/${id}`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
  ...
  const ProductDetailModal = ({
    ...
    useEffect(() => {
      ...
      const PRODUCT_DETAIL_URL = `https://localhost:7259/api/product/order/${orderId}/${product}`;
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span style={{ fontWeight: "400", paddingLeft: 5 }}>
                        {productDetail?.productTemplateName}
                      </span>
                    </p>
                    <p
                      className="title is-5"
                    ...
                    {productDetail?.fabricMaterial && (
                      ...
                      >
                        Vải sử dụng :
                        <span style={{ fontWeight: "400", paddingLeft: 5 }}>
                          {productDetail?.fabricMaterial.name}
                        </span>
                      </p>
                    )}
                  </div>
                ...
                <div>
                  ...
                  >
                    {productDetail?.componentTypeOrders.length !== 0 ? (
                      productDetail?.componentTypeOrders.map(
                        (component, index) => (
                          <div
                            key={index}
                            style={{ marginBottom: "20px", flex: "1 0 45%" }}
                          >
                            <p
                              className="has-text-weight-semibold"
                              style={{ padding: 5, fontSize: 15 }}
                              ...
                              <>
                                ...
                                <div style={{ paddingTop: 5 }}>
                                  {component?.noteObject && (
                                    ...
                                    >
                                      Ghi chú: {component?.noteObject?.note}
                                    </p>
                                  )}
                                </div>
                              </>
  ...
  const items = [
    {
      ...
      label: (
        <p
          className="title is-5"
          style={{ margin: 0, fontSize: 17, fontWeight: "bold" }}
        >
    ...
    },
    {
      ...
      children: (
        ...
        >
          ...
          >
            {orderDetails?.orderMaterials &&
              orderDetails.orderMaterials.map((item, index) => (
                <div
                  key={index}
                  style={{
        ...
        >
            ...
            <p style={{ fontWeight: "bold" }}>
              ...
              <span style={{ fontWeight: "400" }}>
                {orderDetails?.deposit
                  ? formatCurrency(orderDetails?.deposit)
                  : "Không cọc"}
              </span>
            </p>
          </div>
            ...
            <p style={{ fontWeight: "bold" }}>
              ...
              <span style={{ fontWeight: "400" }}>
                {orderDetails?.discountPrice
                  ? formatCurrency(orderDetails?.discountPrice)
                  : "Không"}
              </span>
            </p>
          </div>
          ...
          <div style={{ padding: 10, width: "400px" }}>
            <p style={{ fontWeight: "bold" }}>
              Sau khi giảm:{" "}
              <span style={{ fontWeight: "400" }}>
                {formatCurrency(orderDetails?.afterDiscountPrice)}
              </span>
            </p>
          </div>
