...
function getHoursDifference(startDate) {
  ...
  return hoursDiff >= 0 && hoursDiff < 1
    ? `${Math.floor(hoursDiff * 60) + 1} phút trước`
    : hoursDiff < 24
    ? `${Math.floor(hoursDiff)} giờ trước`
    : `${Math.floor(hoursDiff / 24)} ngày trước`;
}
...
export default function Notification() {
  ...
  const handleOpenChange = (newOpen) => {
    ...
    if (!newOpen) {
      ...
      const GET_NOTIFICATIONS = `https://localhost:7259/api/notification/read-all-notification`;
      try {
                      ...
                      >
                        <p
                          style={{
                            fontWeight: "bold",
                            color: `${
                              notification.isRead ? "#Acacac" : "black"
                            }`,
                          }}
                        >
                          {" "}
